...
class Slice;
extern int getKeyNum(const Slice& s);
static const char* FLAGS_benchmarks =
    "fillseq,"
    "fillrandom,"
    "readwhilewriting,"  // Extra run to allow previous compactions to quiesce
    "readseq,"
    "readrandom,"
...
namespace {
  ...
  class HotspotIntegerGenerator{
      private:
        ...
        int scrambled[config::kSectionNum]={-1};
        Random* rand;
      public:
        HotspotIntegerGenerator(Random* rand,long lowerBound,long upperBound,double hotsetFraction,double hotOpnFraction){
          this->rand = rand;
          //打乱section的顺序,默认hotspot随机生成器会把热点数据都放在前面.我们通过打乱这个顺序,让热点分区可能分布在其他地方.
          //方法是,nextvalue获得了key之后,得到key的secno和secoff,根据secno在scrambled数组中进行随机映射,在根据映射之后的secno和secoff获得新的key
          for(int seci=0;seci<config::kSectionNum;seci++){
            while(true){
              int num = rand->Next()%config::kSectionNum;
              bool equal = false;
          ...
          printf("scrambled :%d,%d,%d,%d,%d\n",scrambled[0],scrambled[1],scrambled[2],scrambled[3],scrambled[4]);
        ...
        long nextValue(){
          long value = 0;
          long r = (double)rand->Next()/RAND_MAX;
          if(r < hotOpnFraction){
            value = lowerBound+labs(rand->Next())%hotInterval;
          }else{
            value = lowerBound+hotInterval+labs(rand->Next())%coldInterval;
          }
          int secno = value/config::kSectionKeynum;
          int secoff = value % config::kSectionKeynum;
          // printf("before value is :%d\n",value);
          // value = scrambled[secno]*config::kSectionKeynum+secoff;
          // printf("after value is :%d\n",value);
          return value;
        }
    };
static const double ZIPFIAN_CONSTANT = 0.99;
static int CURTIME = (int)(time(0));
...
class Benchmark {
 ...
 //测试将ｄｂ当做黑盒，为什么回持有缓存和过滤器的指针呢？
  ...
  void PrintHeader() {
    ...
    PrintEnvironment();
    fprintf(stdout, "version: leveldb original\n");
...
int GetRandomKey(ThreadState* thread){  
    int ran = thread->rand.Next();
    int off = ran%config::kSectionKeynum;
    int secn = (ran%10)>=2?(ran%2):(ran%3)+2;
    int k = secn*config::kSectionKeynum+off;
    return k;
  }
  int GetRandomKey2(ThreadState* thread){  
    int ran = thread->rand.Next();
    int off = ran%config::kSectionKeynum;
    int secn = (ran%10)>=8?(ran%2):(ran%3)+2;
    int k = secn*config::kSectionKeynum+off;
    return k;
  }
  ...
  void DoWrite(ThreadState* thread, bool seq) {
    ...
    // HotspotIntegerGenerator keychooser(&thread->rand,0,FLAGS_num-1,0.4,0.4);
    WriteBatch batch;
    Status s;
    int64_t bytes = 0;
    for (int i = 0; i < num_; i += entries_per_batch_) {
      ...
      for (int j = 0; j < entries_per_batch_; j++) {
        // const int k = GetRandomKey(thread);
        ...
        thread->stats.FinishedSingleOp();
        int seci = k/config::kSectionKeynum;
        s = db_->Write(write_options_, &batch,seci);
      }
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    for (int i = 0; i < reads_; i++) {
      char key[100];
      // const int k = GetRandomKey2(thread);
      const int k = thread->rand.Next() % FLAGS_num;
      // const int k = keychooser.nextValue()%KEYRANGE;
      snprintf(key, sizeof(key), "%016d", k);
  ...
  void ReadWhileWriting(ThreadState* thread) {
    if (thread->tid > 0) {
      ...
      while (true) {
        ...
        // const int k = GetRandomKey(thread);
        const int k = thread->rand.Next() % FLAGS_num;
        // const int k = keychooser.nextValue()%FLAGS_num;
        char key[100];
        snprintf(key, sizeof(key), "%016d", k);
