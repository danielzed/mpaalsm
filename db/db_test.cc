...
TEST(DBTest, RepeatedWritesToSameKey) {
  ...
  const int kMaxFiles = config::kNumLevels + config::kL0_StopWritesTrigger[0];
...
class ModelDB: public DB {
 public:
  ...
  virtual Status Write(const WriteOptions& options, WriteBatch* batch,int seci) {
  ...
  do {
    ...
    for (int step = 0; step < N; step++) {
      ...
      if (p < 45) {                               // Put
        ...
        ASSERT_OK(model.Write(WriteOptions(), &b,057));
        ASSERT_OK(db_->Write(WriteOptions(), &b,057));
      }
