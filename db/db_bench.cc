...
class Benchmark {
 ...
 public:
  ...
  void Run() {
    ...
    while (benchmarks != nullptr) {
      ...
      } else if (name == Slice("testhotcold")){
        fresh_db = true;
        method = &Benchmark::TestHotcold;
      } 
      ...
      } else if (name == Slice("readandwrite")){
        num_threads++;
        method = &Benchmark::ReadAndWrite;
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    // FILE* infolog = fopen("/mnt/f2fs/db1/LOG","a");
    // // Log(db_->GetOption().info_log,"benchmark %s is done\n",name);
    // fputs("benchmark is done\n",infolog);
    // fflush(infolog);
    // fclose(infolog);
    for (int i = 0; i < n; i++) {
      delete arg[i].thread;
  ...
  void TestHotcold(ThreadState* thread){
    ...
    Status s;
    int64_t bytes = 0;
    HotspotIntegerGenerator keychooser(0,KEYRANGE-1,0.4,0.8);
    HotspotIntegerGenerator keychooser_hotcold(0,KEYRANGE-1,0.4,0.2);
    double read_last = 0,write_last = 0,now;
    int read_cnt = 0,write_cnt = 0;
    for (int i = 0; i < reads_*2; i++) {
      int operation_chooser = thread->rand.Next()%20;
      if(operation_chooser <10){
        read_cnt++;
        now = g_env->NowMicros();
        char key[100];
        // const int k = thread->rand.Next() % FLAGS_num;
        const int k = keychooser.nextValue() % KEYRANGE;
        snprintf(key, sizeof(key), "%016d", k);
        if (db_->Get(roptions, key, &value).ok()) {
          found++;
        }
        thread->stats.FinishedSingleOp();
        read_last += g_env->NowMicros()-now;
      }else {
        write_cnt++;
        now = g_env->NowMicros();
        char key[100];
        // const int k = thread->rand.Next() % FLAGS_num;
        const int k = keychooser_hotcold.nextValue() % KEYRANGE;
        snprintf(key, sizeof(key), "%016d", k);
        s = db_->Put(woptions, key, gen.Generate(value_size_));
        if (!s.ok()) {
          printf("error\n");
         fprintf(stderr, "put error: %s\n", s.ToString().c_str());
         exit(1);
        ...
        thread->stats.FinishedSingleOp();
        write_last += g_env->NowMicros()-now;
      }
      
    }
    printf("read have %d,write have %d\n",read_cnt,write_cnt);
    char msg[100];
    snprintf(msg, sizeof(msg), "(%d of %d found)", found, num_);
    thread->stats.AddMessage(msg); 
    ...
    char rate[100];
    snprintf(rate, sizeof(rate), "%6.1f MB/s",
              (bytes / 1048576.0) / write_last);
    extra = rate;
    

    fprintf(stdout, "testhotcold_read : %11.3f micros/op;%s%s\n",
            read_last  / read_cnt,
            (extra.empty() ? "" : " "),
            extra.c_str());
    fprintf(stdout, "testhotcold_write : %11.3f micros/op;%s%s\n",
            write_last  / write_cnt,
            (extra.empty() ? "" : " "),
            extra.c_str());
    fflush(stdout);
  }
  ...
  void ReadAndWrite(ThreadState* thread) {
    if (thread->tid > 0) {
      ReadRandom(thread);
    } else {
      WriteOptions woptions;
      RandomGenerator gen;
      // Special thread that keeps writing until other threads are done.
      int write_cnt = 0,bytes = 0;
      int64_t now,write_last = 0;
      Status s;
